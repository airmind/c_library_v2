...
static inline void mavlink_msg_dronetag_tunneling_send(mavlink_channel_t chan, uint8_t target_network, uint8_t target_system, uint8_t target_component, uint8_t *payload_bytes, int payload_len)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_DRONETAG_TUNNEL_LEN];
    //_mav_put_uint8_t(buf, 2, target_network);
    ...
    _mav_finalize_tunneling_message_chan_send(chan, MAVLINK_MSG_ID_DRONETAG_TUNNEL, buf, MAVLINK_MSG_ID_DRONETAG_TUNNEL_MIN_LEN, payload_len, MAVLINK_MSG_ID_DRONETAG_TUNNEL_CRC, payload_bytes);
#else
    mavlink_dronetag_tunneling_t packet;
    //packet.target_network = target_network;
    packet.target_system = target_system;
    ...
    _mav_finalize_tunneling_message_chan_send(chan, MAVLINK_MSG_ID_DRONETAG_TUNNEL, (char*)&packet, MAVLINK_MSG_ID_DRONETAG_TUNNEL_MIN_LEN, payload_len, MAVLINK_MSG_ID_DRONETAG_TUNNEL_CRC, payload_bytes);
#endif
}
 ...
 */
static inline void mavlink_msg_dronetag_tunneling_send_payload_msg(mavlink_channel_t chan, uint8_t target_network, uint8_t target_system, uint8_t target_component, uint8_t *payload_bytes, int payload_len)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_dronetag_tunneling_send(chan, target_network, target_system, target_component, payload_bytes, payload_len);
#else
    mavlink_dronetag_tunneling_t packet;
    //packet.target_network = target_network;
    packet.target_system = target_system;
